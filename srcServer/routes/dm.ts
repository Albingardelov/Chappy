...
import type { MessageItem, DMBody } from '../data/types.js';
import { authMiddleware } from '../data/auth.js';

const router: Router = express.Router();

// POST /api/dm - skicka direktmeddelande (kräver inloggning)
router.post('/', authMiddleware, async (req: Request<{}, { success: boolean; messageId?: string; message?: string }, DMBody>, res: Response<{ success: boolean; messageId?: string; message?: string }>) => {
    const body = req.body
    ...
    try {
        ...
        console.log(`dm.ts user check fel:`, (error as any)?.message)
...
router.get('/:username', authMiddleware, async (req: Request<{ username: string }>, res: Response<MessageItem[]>) => {
    ...
    try {
        ...
        console.log(`dm.ts GET fel:`, (error as any)?.message)
